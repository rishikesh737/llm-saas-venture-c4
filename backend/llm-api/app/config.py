...
class Settings(BaseSettings):
    ...
    PII_BLOCKING_MODE: str = "FLAG_ONLY"  # ALLOW_ALL, FLAG_ONLY, BLOCK_HIGH_RISK, BLOCK_ALL
...
def validate_settings_on_startup() -> dict:
    ...
    if settings.is_production():
        ...
        if "change" in settings.AUDIT_HMAC_SECRET.lower() or "dev" in settings.AUDIT_HMAC_SECRET.lower():
            warnings.append("AUDIT_HMAC_SECRET appears to be a default value in production!")
    ...
    logger.info(f"PII Detection: {settings.PII_DETECTION_ENABLED} ({settings.PII_BLOCKING_MODE})")
    logger.info(f"Rate Limiting: {settings.RATE_LIMIT_ENABLED} ({settings.DEFAULT_RATE_LIMIT_PER_MINUTE}/min)")
